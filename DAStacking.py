...
# 1) 在函数签名默认超参里添加 ent_coeff（熵正则强度）
def train_model(train_df: pd.DataFrame, test_df: pd.DataFrame,
                ...
                patience_evals=25,
                # 原来: ent_coeff=0.0028
                ent_coeff: float = 0.0045,        # 略加强熵正则，避免单专家权重过大
                reward_lambda: float = 0.8,       # 略降低奖励系数，减缓过拟合节奏
                verbose: bool = False):
    # """
    # 基学习器 + DDPG 门控网络。
    # 状态 = [X_scaled, experts_preds_scaled]; 动作 = 专家权重（归一化 softmax）。
    # 奖励 = - λ * MSE(\hat{y}, y_true) （实际实现中对 y_std^2 做了归一化以稳定数值）。
    # 返回 metrics, model_dict, scalers。
    # """
    ...
    for i, (name, estimator) in enumerate(base_learners):
        preds_scaled = cross_val_predict(
            estimator,
            X_train_scaled,
            y_train_scaled,
            cv=cv,
    ...
    # ---------- 新增：计算基学习器在整个训练集 (full-fit) 上的预测（用于与 test 保持一致的评估） ----------
    # base_train_preds_full: 专家在训练集上的原始量纲预测（shape: n_train x n_experts）
    ...
    # 把这些 full-fit 预测也缩放为 gating 所需的尺度（与 states_train 使用的尺度一致）
    ...
    # 用 full-fit 的专家预测与原始特征构造用于评估的完整训练状态（仅用于评估/打印）
    ...
    # 注意：不要在此处创建 states_train_full_t（device 尚未定义）；将在 device 创建后再转为 tensor
    # -------------------------------------------------------------------------------------------

    # === 2. 为 val/test 构造专家预测与状态 ===
    ...
    # 训练状态：X_train_scaled + OOF（scaled）
    ...
    # 验证 / 测试状态：X_*_scaled + full-fit 专家预测（scaled）
    ...
    # 现在 device 已定义，把用于评估的 full-fit training state 转为 tensor
    ...
    actor_target.load_state_dict(actor.state_dict()); critic_target.load_state_dict(critic.state_dict())
    ...
    critic_criterion = nn.SmoothL1Loss()  # Huber / Smooth L1，更鲁棒
    rb = ReplayBuffer(buffer_size, state_dim, action_dim)

    # 把常用的状态/专家预测提前转为 device 上的 tensor
    ...
    experts_train_t = torch.from_numpy(experts_train_orig.astype(np.float32)).to(device)
    experts_val_t = torch.from_numpy(experts_val_preds.astype(np.float32)).to(device) if experts_val_preds.size else torch.empty((0, n_experts), device=device)
    experts_test_t = torch.from_numpy(experts_test_preds.astype(np.float32)).to(device) if experts_test_preds.size else torch.empty((0, n_experts), device=device)

    # 不使用验证调度器（删除 ReduceLROnPlateau）
    actor_scheduler = None
    critic_scheduler = None

    # 批量化的 actor 推理（支持大样本分批），避免一次性转移过大张量
    def actor_weights(gm, X_input, batch_size_infer=256):
        """
        支持 X_input 为 np.ndarray 或 torch.Tensor（已在 device 上）。
        返回 numpy ndarray (N, n_experts)。
        """
        gm.eval()
        if X_input is None:
            return np.zeros((0, n_experts))
        # 已经是 tensor（可能在 device），直接批量推理
    ...
    # === 预热：顺序遍历环境样本，构造 (s_t,a_t,r_t,s_{t+1},done) ===
    total_steps = 0
        ...
        if total_steps % eval_every == 0:
            with torch.no_grad():
                ...
                if w_train_now.size:
                    ...
                    tr_mse = float(mean_squared_error(y_train, pred_train_now))
                    tr_mae = float(mean_absolute_error(y_train, pred_train_now))
                    tr_r2 = float(r2_score(y_train, pred_train_now))
                else:
                    tr_mse = tr_mae = tr_r2 = float('nan')
                ...
                if states_val_t.shape[0] > 0:
                    ...
                    if w_val_now.size:
                        ...
                        val_mse = float(mean_squared_error(y_val, pred_val_now))
                        val_mae = float(mean_absolute_error(y_val, pred_val_now))
                        val_r2 = float(r2_score(y_val, pred_val_now))
                    else:
                        val_mse = val_mae = val_r2 = float('nan')
                else:
                    val_mse = val_mae = val_r2 = float('nan')
            ...
            # 早停：优先按 val_mae，如果没有 val（极小数据集）则退回 train_mae
    ...
    if best_actor_state is not None:
        # 训练循环结束后，如果之前一直在用 '\r'，这里也补一个换行，保证后续输出正常排版
        if verbose:
            print()
        actor.load_state_dict(best_actor_state)

    # === 5. 最终使用确定性 actor 在 train/val/test 上预测 ===
    ...
    def safe_mape(y_true, y_pred, eps: float = 1e-8):
        """
        返回百分比形式的 MAPE（0~100），对 y_true≈0 做截断以避免除零。
        """
...
if __name__ == "__main__":
    # 从预划分的 CSV 文件读取数据（只使用 train/test）
